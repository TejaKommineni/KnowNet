...
The output of the pattern detection step is a set of clusters. We explored these clusters to better understand the grouping of hosts. A detailed explanation of our observations is provided in the evaluation section. On a higher level, we observed that each cluster exhibits a unique behavior. That is all the hosts in a cluster exhibit similar behavior. We labeled these behvaiors in an human understandable format and observed how these behaviors are changing over time using the following procedure outlined.

\begin{itemize}
	\item We chose a reference day for labeling the clusters and assigned labels by manually inspecting contents of the cluster.
	
	\item Now, we use this reference day labeling and identify the behaviors on any other day.
\end{itemize} 
